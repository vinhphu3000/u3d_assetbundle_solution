...
public class BuildImpl
{
    ...
    public static List<MaterialTexturePair> RemovePrefabMatTex(GameObject prefab)
    {
        ...
        var pmh = prefab.GetComponent<PrefabMaterialHolder>();
        if (pmh != null)
        {
            GameObject.DestroyImmediate(pmh, true);
        }
        pmh = prefab.AddComponent<PrefabMaterialHolder>();//为场景根节点增加PrefabMaterialHolder组件，用以保存场景对图片的依赖
        pmh.rendersMatTexInfo = new List<PrefabMaterialHolder.RenderMatTexPair>();
        ...
        for (int i = 0; i < renders.Count; i++)
        {
            ...
            {
                var rmp = new PrefabMaterialHolder.RenderMatTexPair();
                rmp.renderObj = renders[i];
                //保存每个render下所有材质的贴图信息
                var matis = new List<PrefabMaterialHolder.MaterialAllTextureInfo>();

                for (int j = 0; mats != null && j < mats.Length; j++)
                {
                    //保存每个材质的所有纹理信息
                    var mtis = new List<PrefabMaterialHolder.MaterialTextureInfo>();
                    //得到每个材质的纹理信息 
                    for (int si = 0; mats[j] != null && si < ShaderUtil.GetPropertyCount(mats[j].shader); si++)
                    {
                        ...
                        if (st == ShaderUtil.ShaderPropertyType.TexEnv)
                        {
                            ...
                            //保存到打包信息对象 
                            mtis.Add(new PrefabMaterialHolder.MaterialTextureInfo { attribute = strProperty, tex2dName = tex.name.ToLower() });
                        }
                    }
                    if (mtis.Count > 0)
                    {
                        matis.Add(new PrefabMaterialHolder.MaterialAllTextureInfo { matIndex = j, matTexInfos = mtis.ToArray() });
                    }
                }
                if (matis.Count > 0)
                {
