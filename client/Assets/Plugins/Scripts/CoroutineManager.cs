...
public class CoroutineManager : MonoBehaviour
{
    ...
    /// </summary>
    /// <param name="id"></param>
    public void stopCoroutine(Int64 id)
    {
    ...
    public long delayedCall(float delayedTime, UnityAction callback)
    {
        return startCoroutine(delayedCallImpl(delayedTime, callback));
    }

    private IEnumerator delayedCallImpl(float delayedTime, UnityAction callback)
    {
        if (delayedTime >= 0)
    ...
    public long delayedCall(float delayedTime, UnityAction<object> callback, object param)
    {
        return startCoroutine(delayedCallImpl(delayedTime, callback, param));
    }

    private IEnumerator delayedCallImpl(float delayedTime, UnityAction<object> callback, object param)
    {
        if (delayedTime >= 0)
            yield return new WaitForSeconds(delayedTime);
        callback(param);
